...
#        current_pixel_number += 1
    ...
    for current_row in range(0, height-1):
        xposition=width/2
        remaining_tries=0.01*width
        # Schaue nach Rechts
        while (remaining_tries>0 and xposition<width):
            pixel=imgdata[current_row][xposition]
            if (19<pixel[0]<90 and pixel[1]>5 and 50<pixel[2]):             # Dies erkennt das Gelb
                yellow_pixel_amount += 1                                    # Anzahl der gelben Pixel erhoehen
                yellow_pixel_per_row[current_row] +=1                       # Anzahl der gelben Pixel in der entsprechenden Reihe erhoehen
            else:
        ...
        xposition=(width/2)-1
        remaining_tries=0.01*width
        while (remaining_tries>0 and xposition>=0):
            pixel=imgdata[current_row][xposition]
            if (19<pixel[0]<90 and pixel[1]>5 and 50<pixel[2]):             # Dies erkennt das Gelb
                yellow_pixel_amount += 1                                    # Anzahl der gelben Pixel erhoehen
                yellow_pixel_per_row[current_row] +=1                       # Anzahl der gelben Pixel in der entsprechenden Reihe erhoehen
...
#        current_pixel_number += 1
    ...
    # Schritt 2: Alle Pixel des Bilds durchlaufen
    imgdata = img_orig.getdata()
    img_rot_data = img_copy.getdata()
    ...
    for current_row in range(0, height-1):
        ...
        # Schaue nach Rechts
        while (remaining_tries>0 and xposition<width):
            current_position=int((current_row*width)+xposition)
            pixel=imgdata[current_position]
            if (19<pixel[0]<90 and pixel[1]>5 and 50<pixel[2]):             # Dies erkennt das Gelb
                yellow_pixel_amount += 1                                    # Anzahl der gelben Pixel erhoehen
                yellow_pixel_per_row[current_row] +=1                       # Anzahl der gelben Pixel in der entsprechenden Reihe erhoehen
                img_rot_data[current_position]=leuchtend_rot_hsv
            else:
                remaining_tries -= 1
                xposition += 1
